...
int main(int argc, char** argv) {
    ...
    JsonLoader::load("secrets", projectPath + JsonLoader::toStringMap("main").at("SECRETS_CONFIG"));
    auto secrets = JsonLoader::toStringMap("secrets");

    std::string valuationDate = "17-01-2025";

    // === Step 1: Retrieve trades from DB ===
    print_utils::printBoxedLabel("📡 Trade Retrieval");
    TradesStore store(projectPath + JsonLoader::toStringMap("main").at("DB_PATH"));
